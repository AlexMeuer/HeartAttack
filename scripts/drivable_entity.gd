...
func thrust(delta):
	var new_vel = velocity + get_forward() * _THRUST * delta
	velocity = new_vel.clamped(_MAX_SPEED)
...
func brake(delta):
	...
	if get_speed() - decel.length() < 0:
		...
		velocity -= get_forward() * _THRUST * delta
...
func turn_toward(point, delta):
	...
	velocity = get_speed() * get_forward().rotated(rotate_dir * rotate_amount)